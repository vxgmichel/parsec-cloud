...
class LocalStorage(BaseAsyncComponent):
    ...
    def flush_user_pubkey(self, user_id, pubkey):
        cur = self.conn.cursor()
        cur.execute("INSERT OR REPLACE INTO users (user_id, pubkey) VALUES (?, ?)", (user_id, pubkey))
        self.conn.commit()

    def fetch_device_verifykey(self, user_id, device_name):
        cur = self.conn.cursor()
        cur.execute("SELECT verifykey FROM devices WHERE user_id=? AND device_name=?", (user_id, device_name))
        res = cur.fetchone()
        if res is not None:
            return res[0]
    ...
    def flush_device_verifykey(self, user_id, device_name, verifykey):
        cur = self.conn.cursor()
        cur.execute("INSERT OR REPLACE INTO devices (user_id, device_name, verifykey) VALUES (?, ?, ?)", (user_id, device_name, verifykey))
        self.conn.commit()
